...
const RemoveFromGroupButton = () => {
	...
	return (
			...
			<Row hoverEffect={true}>				
					<Button label="Remove from group" onClick={clickHandler} tooltip="To remove a member from a group, click a member from a group, then click this button." />
			</Row>
	);
};
...
const CustomPhaseEditorButton = () => {
	const clickHandler = () => {
		setPanelState(MainPanelState.CustomPhase);
	};
	return (
		<Row hoverEffect={true}>
...
const BackButton = ({ previousState }: { previousState?: number }) => {
	...
	return (
		<Row hoverEffect={true}>
			<Button label="Back" onClick={() => setPanelState(targetState)} />
		</Row>
	);
};
					...
					{displayedGroup ? (
						<>
							...
							{displayedGroup.members && displayedGroup.members.length > 0 ? (
								...
								<FocusDisabled>
									<Tooltip tooltip="Click a member name to select the junction, and click the foldout to show its options." direction="right">
										<MemberFoldout
											key={`${member.index}-${member.version}`}
											member={member}
											onMemberClick={handleMemberClick}
											currentGroup={displayedGroup}
											isLockstep={displayedGroup.isCoordinated && !displayedGroup.greenWaveEnabled}
										/>
									</Tooltip>
								</FocusDisabled>
