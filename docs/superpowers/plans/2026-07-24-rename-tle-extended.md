...
**Architecture:** Add a spaced display name and a separate `TrafficLightEnhancementsExtended` filesystem name alongside the existing internal module ID, point C# and UI deployment at the filesystem-safe folder, rename release staging, and update visible product labels while preserving assembly, namespace, localization-key, and serialization identities.
...
- [ ] Update release staging so the archive contains the `TrafficLightEnhancementsExtended` folder.
...
- [ ] Build and install into `Mods\TrafficLightEnhancementsExtended`.
